     ...
     extends VariateVec (stream)
{
    ...
    def igen: Array[Int] = gen.map(x => x.toInt)
 ...
 */
/*
case class NormalVec (mu: Array[Double], cov: Array[Array[Double]], stream: Int = 0)
     extends VariateVec (stream)
{
    ...
    def pf (z: Array[Double]): Double =
    {
        val n    = z.length.toDouble                        // n-dimensional vectors
        val z_mu = (z zip mu).map(x => x._1 - x._2)                                // subtract mean

	//Refactored to here, after which we need the CF matrix to finish...
    ...
    def gen: Array[Double] =
    {
        val z = Array.fill(mu.length)(0) 
        for (i <- 0 until mu.length) z(i) = normal.gen
 ...
 *  @param stream  the random number stream
 */
//We don't have the math.Combinatorics package any more so we can't build this...
/*
case class Dir (alpha: Array[Double], stream: Int = 0)
{
    import scalation.math.Combinatorics.gammaF

    private val a_sum = alpha.sum

    def mean: Array[Double] = alpha.map(x=>x/ a_sum)
    ...
    def pf (z: Array[Double]): Double =
    {
        ...
        for (i <- alpha.indices) {
            val a_i = alpha(i)
            prod *= (math.pow(z(i),(a_i-1.0))) / gammaF (a_i)
        } // for
        prod * gammaF (a_sum)
 ...
 */
/*
case class PermutedVecD (x: Array[Double], stream: Int = 0)
     extends VariateVec (stream)
{
    private val mu  =  x.sum / x.length.toDouble		// mean
    private val rng = Randi0 (x.length-1, stream)          // random integer generator

    def mean: Array[Double] = Array.fill(x.length)(mu)

    //def pf (z: Array[Double]): Double = 1.0 / fac (x.dim)
    def pf = 1.0					// dummy value until fac is returned

    def gen: Array[Double] = 
    {
        val y = x.clone()				// copy vector x
 ...
 */
/*
case class PermutedVecI (x: Array[Int], stream: Int = 0)
     extends VariateVec (stream)
{
    _discrete = true

    private val mu  = x.sum / x.length.toDouble            // mean
    private val rng = Randi0 (3 * x.length, stream)        // random integer generator

    def mean: Array[Double] = Array.fill (x.length)(mu)

    //def pf (z: Array[Double]): Double = 1.0 / fac (x.dim)
    def pf = 1.0						//dummy value until we fix

    def gen: Array[Double] = igen.map(x => x.toDouble)
     ...
     extends VariateVec (stream)
{
    ...
    def mean: Array[Double] = Array.fill (samp)(mu)

    def pf (z: Array[Double]): Double = 1.0 / fac (pop)
  
    def gen: Array[Double] = igen.map(x=>x.toDouble)

    def igen: Array[Int] =
    {
        val y = Array.range (0, pop)                  // generate vector containing 0, 1, ... pop-1
        for (i <- 0 until samp) {
     ...
     extends VariateVec (stream)
{
    ...
    def mean: Array[Double] = Array.fill (dim)(mu)

    //def pf (z: Array[Double]): Double = 1.0 / (max - min) ~^ dim
    def pf (z: Array[Double]): Double = scala.math.pow(1.0/(max-min),dim)

    def igen: Array[Int] = gen.map(x=>x.toInt)

    def gen: Array[Double] =
    {
         (for (i <- 0 until dim) yield if (rn.gen < density) rng.gen else 0.0)
    } // gen

    def repgen: Array[Double] =
    {
        val v   = Array.ofDim(dim)
        var cnt = 0
        while (cnt < dim) {
            val x   = rng.gen                    // value
     ...
     extends VariateVec (stream)
{
    ...
    def pf (z: Array[Double]): Double = NaN

    def gen: Array[Double] = sgen.map(x=>x.toDouble)
     ...
     extends VariateVec (stream)
{
    ...
    val mean: Array[Double] = VectorD.fill (p.length)(dice.mean)
    
    def pf (z: Array[Double]): Double =
    {
        val x = z.map(zz=>zz.toInt)
        var prob = choose (n, x(0), x(1)).toDouble
        for (j <- 2 until p.length) prob /= fac (x(j))
        prob
    } // pf
    
    def pf (z : Array[Double]): Double = 0.0
    
    def gen: Array[Double] = igen.map(x=>x.toDouble)

    def igen: Array[Int] =  (for (i <- p.indices) yield dice.igen)
